@extends('layout')

@section('content')
    <main>
        <!--? Hero Start -->
        <div class="slider-area2">
            <div class="slider-height2 d-flex align-items-center">
                <div class="container">
                    <div class="row">
                        <div class="col-xl-12">
                            <div class="hero-cap hero-cap2 pt-20 text-center">
                                <h2>OJ's Impact </h2>
                                <p class="pt-2">Saddled with the responsibility of overseeing the Basic Education
                                    Quality Assurance (BEQA) </p>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
        <!-- Hero End -->
        <!-- Intro -->
        <section class="ftco-section mt-md-20">
            <div class="container">
                <div class="row d-flex justify-content-center">
                    <div class="col-md-6 d-flex ftco-animate">
                        <div class="img img-about align-self-stretch shadow-lg"
                             style="background-image: url(assets/img/gallery/public-profile.jpg); width: 100%; height: 400px; border-radius: 5px;"></div>
                    </div>
                    <div class="col-md-6 pl-md-5 ftco-animate d-flex justify-content-center flex-column">
                        <p>
                            <strong> Mr. Owolabi Jamiu Falana</strong> was appointed in August, 2022 by Governor Babajide
                            Sanwo-Olu to serve as Board Member in charge of Basic Education Quality
                            Assurance, Lagos State Universal Basic Education Board.
                        </p>
                    </div>
                </div>
            </div>
        </section>
        <!-- Brief -->
        <section class="ftco-counter ftco-intro" id="section-counter">
            <div class="container">
                <div class="row no-gutters">
                    <div class="col-md-6 d-flex justify-content-center counter-wrap ftco-animate">
                        <div class="block-18 color-1 align-items-stretch">
                            <div class="text">
{{--                                <span class="text-black">Completed Assignments</span>--}}
{{--                                <strong class="number text-black" data-number="2805">0</strong>--}}
                                <p class="text-black">At Lagos SUBEB, following Mr. Governor's footsteps of efficient
                                    service delivery, he has been able to achieve the following;</p>
                            </div>
                        </div>
                    </div>
                    <div class="col-lg-6 d-flex justify-content-center counter-wrap ftco-animate">
                        <div class="block-18 color-2 align-items-stretch">
                            <div class="text">
                                <p class="text-black">1. He spearheaded the comprehensive assessment of all 20 Local
                                    Government Education
                                    Authorities across Lagos State, delivering on-the-spot solutions to challenges and
                                    providing practicable recommendations for executive consideration. A rst by any
                                    board
                                    member.</p>
                            </div>
                        </div>
                    </div>
                    <div class="col-md-6 d-flex justify-content-center counter-wrap ftco-animate">
                        <div class="block-18 color-3 align-items-stretch">
                            <div class="text">
                                <p class="text-black">2. He led his team to successfully actualize the digitalization
                                    and transition program of EKO
                                    EXCEL to Basic Education Quality Assurance (BEQA) Department, to ensure the
                                    sustainability of the Lagos State Government's signicant investment in the basic
                                    education
                                    sector..</p>
                            </div>
                        </div>
                    </div>
                    <div class="col-md-6 d-flex justify-content-center counter-wrap ftco-animate">
                        <div class="block-18 color-1 align-items-stretch">
                            <div class="text">
                                <p class="text-black">3. He reinvigorated the team, fostering a culture of innovation
                                    and collaboration, resulting in
                                    enhanced support for key education initiatives.</p>
                            </div>
                        </div>
                    </div>
                    <div class="col-md-6 d-flex justify-content-center counter-wrap ftco-animate">
                        <div class="block-18 color-2 align-items-stretch">
                            <div class="text">
                                <p class="text-black">4. He reorganized and revitalized the transition from EKO EXCEL
                                    Team to BEQA
                                    Department, resulting in enhanced monitoring of daily school activities and better
                                    learning
                                    outcomes.</p>
                            </div>
                        </div>
                    </div>
                    <div class="col-md-6 d-flex justify-content-center counter-wrap ftco-animate">
                        <div class="block-18 color-3 align-items-stretch">
                            <div class="text">
                                <p class="text-black">5. He successfully trained over 200 ofcers on critical areas like
                                    Record Keeping, School
                                    Management, and Effective Monitoring, contributing to overall educational
                                    improvements.</p>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </section>
        <!-- gallary impacts mobile -->
        <section class="d-md-none d-flex">
            <div class="container">
                <div class="row">
                    <div class="col-lg-4">
                        <div class="blog_right_sidebar">
                            <aside class="instagram_feeds">
                                <ul class="instagram_row flex-wrap">
                                    <li>
                                        <div href="/assets/img/impacts/15.jpg"
                                             class="gallery image-popup d-flex justify-content-center align-items-center img ftco-animate"
                                             style="background-image: url('assets/img/impacts/15.jpg');">
                                        </div>
                                    </li>
                                    <li>
                                        <div href="/assets/img/impacts/4.jpg"
                                             class="gallery image-popup d-flex justify-content-center align-items-center img ftco-animate"
                                             style="background-image: url('assets/img/impacts/4.jpg');">
                                        </div>
                                    </li>
                                    <li>
                                        <div href="/assets/img/impacts/5.jpg"
                                             class="gallery image-popup d-flex justify-content-center align-items-center img ftco-animate"
                                             style="background-image: url('assets/img/impacts/5.jpg');">
                                        </div>
                                    </li>
                                    <li>
                                        <div href="/assets/img/impacts/1.jpg"
                                             class="gallery image-popup d-flex justify-content-center align-items-center img ftco-animate"
                                             style="background-image: url('assets/img/impacts/1.jpg');">
                                        </div>
                                    </li>
                                    <li>
                                        <div href="/assets/img/impacts/7.jpg"
                                             class="gallery image-popup d-flex justify-content-center align-items-center img ftco-animate"
                                             style="background-image: url('assets/img/impacts/7.jpg');">
                                        </div>
                                    </li>
                                    <li>
                                        <div href="/assets/img/impacts/8.jpg"
                                             class="gallery image-popup d-flex justify-content-center align-items-center img ftco-animate"
                                             style="background-image: url('assets/img/impacts/8.jpg');">
                                        </div>
                                    </li>
                                </ul>
                            </aside>
                        </div>
                    </div>
                </div>
            </div>
        </section>
        <!-- gallary impacts -->
        <section class="ftco-gallery mb-50">
            <div class="d-md-flex d-none">
                <a href="/assets/img/impacts/3.jpg"
                   class="gallery image-popup d-flex justify-content-center align-items-center img ftco-animate"
                   style="background-image: url('assets/img/impacts/3.jpg');">
                </a>
                <a href="assets/img/impacts/15.jpg"
                   class=" d-md-flex d-none gallery image-popup d-flex justify-content-center align-items-center img ftco-animate"
                   style="background-image: url('assets/img/impacts/15.jpg');">
                </a>
                <a href="assets/img/impacts/5.jpg"
                   class=" d-md-flex d-none gallery image-popup d-flex justify-content-center align-items-center img ftco-animate"
                   style="background-image: url('assets/img/impacts/5.jpg');">
                </a>
                <a href="assets/img/impacts/14.jpg"
                   class=" d-md-flex d-none gallery image-popup d-flex justify-content-center align-items-center img ftco-animate"
                   style="background-image: url('assets/img/impacts/14.jpg');">
                </a>
            </div>
            <div class="d-md-flex d-none">
                <a href="assets/img/impacts/7.jpg"
                   class="gallery image-popup d-flex justify-content-center align-items-center img ftco-animate"
                   style="background-image: url('assets/img/impacts/7.jpg');">
                </a>
                <a href="assets/img/impacts/8.jpg"
                   class="gallery image-popup d-flex justify-content-center align-items-center img ftco-animate"
                   style="background-image: url('assets/img/impacts/8.jpg');">
                </a>
                <a href="assets/img/impacts/2.jpg"
                   class="gallery image-popup d-flex justify-content-center align-items-center img ftco-animate"
                   style="background-image: url('assets/img/impacts/2.jpg');">
                </a>
                <a href="assets/img/impacts/4.jpg"
                   class="gallery image-popup d-flex justify-content-center align-items-center img ftco-animate"
                   style="background-image: url('assets/img/impacts/4.jpg');">
                </a>
            </div>
            <div class="d-md-flex d-none">
                <a href="assets/img/impacts/10.jpg"
                   class="gallery image-popup d-flex justify-content-center align-items-center img ftco-animate"
                   style="background-image: url('assets/img/impacts/10.jpg');">
                </a>
                <a href="assets/img/impacts/11.jpg"
                   class=" d-md-flex d-none gallery image-popup d-flex justify-content-center align-items-center img ftco-animate"
                   style="background-image: url('assets/img/impacts/11.jpg');">
                </a>
                <a href="assets/img/impacts/12.jpg"
                   class=" d-md-flex d-none gallery image-popup d-flex justify-content-center align-items-center img ftco-animate"
                   style="background-image: url('assets/img/impacts/12.jpg');">
                </a>
                <a href="assets/img/impacts/13.jpg"
                   class=" d-md-flex d-none gallery image-popup d-flex justify-content-center align-items-center img ftco-animate"
                   style="background-image: url('assets/img/impacts/13.jpg');">
                </a>
            </div>
        </section>
        <!-- gallary impacts mobile -->
        <section class="d-md-none d-flex">
            <div class="container">
                <div class="row">
                    <div class="col-lg-4">
                        <div class="blog_right_sidebar">
                            <aside class="instagram_feeds">
                                <ul class="instagram_row flex-wrap">
                                    <li>
                                        <div href="assets/img/impacts/10.jpg"
                                             class="gallery image-popup d-flex justify-content-center align-items-center img ftco-animate"
                                             style="background-image: url('assets/img/impacts/10.jpg');">
                                        </div>
                                    </li>
                                    <li>
                                        <div href="assets/img/impacts/11.jpg"
                                             class=" d-md-flex d-none gallery image-popup d-flex justify-content-center align-items-center img ftco-animate"
                                             style="background-image: url('assets/img/impacts/11.jpg');">
                                        </div>
                                    </li>
                                    <li>
                                        <div href="assets/img/impacts/12.jpg"
                                             class=" d-md-flex d-none gallery image-popup d-flex justify-content-center align-items-center img ftco-animate"
                                             style="background-image: url('assets/img/impacts/12.jpg');">
                                        </div>
                                    </li>
                                </ul>
                            </aside>
                        </div>
                    </div>
                </div>
            </div>
        </section>

@endsection
